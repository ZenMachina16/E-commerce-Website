...
app.post("/submitForm", (req, res) => {
  try {
    ...
    db.query(
      "INSERT INTO wsschema (email, fname, lname, age, password) VALUES (?, ?, ?, ?, ?)", 
...
try {
  ...
  db.query(
    "SELECT * FROM wsschema WHERE email = ? AND password = ?",
    [email, password],
    (err, result) => {
      if (err) {
...
app.post("/changedata",(req,res)=>{
  try {
    ...
    db.query(
      `UPDATE wsschema SET fname="${fname}", lname="${lname}", age="${age}" WHERE email = ?`,
      [email],
      (err, result) => {
        if (err) {
          console.error("Error querying data from MySQL:", err);
...
app.delete("/deleteProfile", (req,res)=>{
  ...
  db.query(
    "DELETE FROM wsschema WHERE email = ?",
    [email],
    (err, result) => {
      if (err) {
        console.error("Error querying data from MySQL:", err);
...
app.post("/addToCart", (req, res) => {
  try {
    ...
    // Insert the data into the MySQL table for the cart
    db.query(
      "INSERT INTO addtocart (product_name, product_price) VALUES (?, ?)",
